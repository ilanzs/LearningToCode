...
class Cell():
    ...
    def highlight(self):
        x = self.i * w
        y = self.j * w
        pygame.draw.rect(WIN, BLUE, pygame.Rect(x + 1, y + 1, w - 1, w - 1))
...
def main():
    ...
    while run:
        ...
        while len(stack) > 0:
            ...
            # STEP 2.2
            if current.checkNeighbors() != None:
                stack.append(current)
            ...
            if len(stack) == len(grid):
                break

            # color the last cell
            pygame.draw.rect(WIN, RED, pygame.Rect(WIDTH - w + 1, HEIGHT - w + 1, w - 1, w - 1))
